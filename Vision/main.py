    ...
    if len(contours) >= 2:
        ...
        for i in range(len(biggestCnts) - 1):
            ...
            if (np.sign(tilt1) != np.sign(tilt2)):
                ...
                # left
                # epsilonL = cv2.arcLength(leftContour, True)
                # approxL = cv2.approxPolyDP(leftContour, epsilonL, True)
                #
                # # left
                ...
                if [centerOfTargetX, yawToTarget] not in targets:
                    targets.append([centerOfTargetX, yawToTarget])
...
if __name__ == "__main__":
    ...
    networkTable = NetworkTables.getTable('SmartDashboard/Vision')
